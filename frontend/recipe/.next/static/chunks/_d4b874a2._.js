...
;
;
const BACKEND_BASE = ("TURBOPACK compile-time value", "http://localhost:3000") || '';
        ...
        "RecipeInfoPage.useEffect": ()=>{
            ...
            async function load() {
                try {
                    ...
                    setRecipe(main);
                    const listRes = await fetch(`${BACKEND_BASE}/recipes?category=${encodeURIComponent(main.category)}`, {
                        cache: 'no-store'
                    });
                    ...
                    if (!listData.meals) {
                        ...
                        const lookups = await Promise.all(listData.meals.map({
                            "RecipeInfoPage.useEffect.load": async (item)=>{
                                const r = await fetch(`${BACKEND_BASE}/recipes/${item.idMeal}`, {
                                    cache: 'no-store'
                                });
                                if (!r.ok) return null;
                        ...
                        }["RecipeInfoPage.useEffect.load"]));
                        const filtered = lookups.filter({
                            "RecipeInfoPage.useEffect.load.filtered": (x)=>x !== null && x.id !== main.id
                        }["RecipeInfoPage.useEffect.load.filtered"]);
                        setRelated(filtered);
                    }
                } catch (err) {
...
//# sourceMappingURL=_d4b874a2._.js.map